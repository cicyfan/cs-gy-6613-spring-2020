...
* Utitlity theory describes what the agent wants
* Decision theory puts the two together to describe what the agent should do i.e $ = $ Utitlity Theory + Probability Theory

----

### Decision Networks

* Bayseian networks are used to represent knowlege in an uncertain domain 
...
* Utility of a state is the immediate reward of the state plus the expected discounted utitlity of the next state due to the action taken  
...
\begin{eqnarray\*}
U(S_{33})  & =  & R(S_{33}) +  (P(S_{43} | S_{33}, \rightarrow)U(S_{43}) + ... \\\\
& =  & R(S_{33}) +  (P(S_{43} | S_{33}, \rightarrow) U(S_{43}) + (P(S_{33} | S_{33}, \rightarrow)  R(S_{33}) + ... \\\\
& =  & R(S_{33}) +  (P(S_{43} | S_{33}, \rightarrow)  U(S_{43}) + (P(S_{33} | S_{33}, \rightarrow)  R(S_{33}) \\\\
& & + (P(S_{32} | S_{33}, \rightarrow)  U(S_{32})
\end{eqnarray\*}
* Substituting we get:

 $U(S_{33}) = R(S_{33}) + \gamma ( (0.8 \times 1) + (0.1 \times R(S_{33}) + (0.1 \times U(S_{23}) )$
...
* We can write this as $U^{*}(s) = R(s) + \gamma max_{a} (\sum_{s^{'}} P(s^{'}| s,a)U(s^{'}))$
* The optimal policy (which recommends $a$ that maximizes U) is given by:
$\pi^{*}(s) = argmax_{a}(\sum_{s^{'}} P(s^{'}| s,a)U(s^{'}))$
...
* Steps:
	...
	* The iteration step is called **Bellman Update**: $U_{t+1}(s) \leftarrow R(s) + \gamma max_{a} (\sum_{s^{'}} P(s^{'}| s,a)U(s^{'})) \forall s \epsilon S$. This step is repeated and updated
...
& \leftarrow & -0.04 + P(S_{43}|S_{33},\rightarrow)U(S_{43})+P(S_{33}|S_{33},\rightarrow)U(S_{33})+P(S_{32}|S_{33},\rightarrow)U(S_{32}) \\\\
...
![(A)Initial utility estimates for iteration 2 (B) States with next state positive utility](./images/val-iter-step2-initial.png)<!-- .element width="500px" -->


\begin{eqnarray\*}
    U_{t+1}(S_{33}) & \leftarrow &   -0.04 + P(S_{43}|S_{33},\rightarrow)U(S_{43})+P(S_{33}|S_{33},\rightarrow)U(S_{33})+P(S_{32}|S_{33},\rightarrow)U(S_{32}) \\\\
 & \leftarrow &  -0.04 + 0.8 \times 1 + 0.1 \times 0.76 + 0.1 \times 0 = 0.836 \\\\
   ...
   & \leftarrow & = -.04 + 0.8 \times 0.76 = 0.568 \\\\
    U_{t+1}(S_{32}) & \leftarrow &  -0.04 + P(S_{33}|S_{32},\uparrow)U(S_{33})+P(S_{42}|S_{32},\uparrow)U(S_{42})+P(S_{32}|S_{32},\uparrow)U(S_{32}) \\\\
...
* Rate of convergence depends on the maximum reward value and more importantly on the discount factor $\gamma$. Convergence time increases as the discount factor is closer to 1.  
* The policy that get from coarse estimates is close ptimal policy long before $U$ has converged.
...
* The steps are as follows:
	...
	2. Compute the utility of value of the policy $U^{\pi}$(by solving a system of linear equations)
	3. Update $\pi$ to be the greedy policy with respect to $U^{\pi}$ i.e
$\pi(s) \leftarrow argmax_{a} \sum_{s^{'}} P(s^{'}|s,a)U^{\pi}(s^{'})$
	4. If the policy changed then return to step $2$
![Policy iteration(source: sutton, chap 4)](./images/policy-iter.PNG)<!-- .element width="150px" -->
...
* Find the value function based on initial random policy using $U(s) = R(s) + \sum_{s^{'}} P(s^{'}| s,a)U(s^{'})$ 
\begin{eqnarray\*}
U(S_{1}) & = -1 + \frac{1}{2}U(S_{1}) + \frac{1}{4}U(S_{2}) + \frac{1}{4}U(S_{5}) = -1 \\\\
\ldots & \\\\
U(S_{9}) & = -1 + \frac{1}{2}U(S_{8}) + \frac{1}{4}U(S_{5}) + \frac{1}{4}U(S_{13}) + \frac{1}{4}U(S_{10}) = -1 \\\\
\ldots 
\end{eqnarray\*}
* Next we compute the policy:
\begin{eqnarray\*}
\pi(S_{4}) & =  max[\frac{1}{4}U(S_{term})| \uparrow, 
 & \frac{1}{4}U(S_5)| \rightarrow, 
 & \frac{1}{4}U(S_4)| \leftarrow, 
 & \frac{1}{4}U(S_8), \downarrow] \\\\
\pi(S_{6}) & = max[\frac{1}{4}U(S_2)| \uparrow, 
\frac{1}{4}U(S_7)| \rightarrow, 
& \frac{1}{4}U(S_5)| \leftarrow, 
\frac{1}{4}U(S_{10}), \downarrow ] 
\end{eqnarray\*}
...
* Continuing to alternate between policy evaluation and improvement, we see that the policy converges to the optimal policy after 3 iteration while the value function takes a while to converge (even after 10 iterations it has not converged!)
...
* The belief state $b$ also satifies the Marokov property

----

### POMDP versus other models
 
* MDP versus POMDP versus Markov Chain versus Hidden Markov Model:

| Markov Models    |  | Do we have control over state transitions| |  
|----|------|-------|--------------------| 
|    |             | No   | Yes   |                    
| **States completely observable** | Yes | Markov Chain | MDP |
| **States completely observable**    | No | Hidden Markov Models | POMDP |
...
* each policy is a conditional plan condition on belief $b$ 
...
$U_{t+1}(b) \leftarrow \max_{a}[ \sum_s b(s)R(s,a) +\gamma \sum_eP(e|b,a)U(\tau(e,b,a))$. Where $\tau(e,b,a)$ is the transition function for the belief state.. Where $\tau(e,b,a)$ is the transition function for the belief state.
...
* Determine the expected utility for each possible action for different belief distributions
* We would pick "listen" as it has the highest expected utiltiy 
* How low should $\rho$ go so that the utitliy is better than picking "listen"
* Find $x \ni \rho \times -100 + (1-\rho) \times 10 < -1$
* Solving we get $\rho < 0.1$. This means that if that if $0 < b(left) < 0.1$ then choose "left. This range is called the **belief interval** for which we would select "left".
...
![(A) Utility as function of plan (B) Value function](./images/tiger-1step-value.png)<!-- .element width="600px" -->
* How about if you were given 2 chances? i.e. $t=2$.
	* It turns out that the optimal policy for the first step is to always "listen" irrespective of the belief state distribution. 
	* The reason is that if you opened the door, the tiger would be randomly placed behind one of the doors and
the agent’s belief state would be reset to (0.5, 0.5). 
	* So after opening a door, the agent would be left with no information about the tiger’s location and with one action remaining.
	* We just saw that with one step to go and b = (0.5, 0.5) the best thing to do is listen.
Therefore, if the agent opens a door when t = 2, it will listen on the last step 
...
* What is learned:
	 ...
	 * utlity information indicating desirability of states
...
* Utility functions $U^{\pi}(s)$ provide expected utility or return or reward of a state by executing a given policy $\pi$
* $V^{\pi}=U^{\pi}(s)$  
...
* You get the value function by taking the expection of the action-value function over the set of actions i.e. $V^{\pi}(s) = E_{a}[Q(s,a)]$
...
* CONS
	* no gaurantee of convergence to optimal policy
...
* agent learnds a model of the environment dynamics
...
* CONS:
	...
	* predictions depend on teh accuracy of the model 
...
* Neural networks learn functions $f_{\theta}(x)$ which map input $x$ to output $y$. The weights of the neural network are represented by $\theta$.
	...
	* Loss function $L(f_{\theta}(x),y)$ captures the different between the target $y$ and the predicted network output $f_{\theta}(x)$. This loss function needs to be minimized.
